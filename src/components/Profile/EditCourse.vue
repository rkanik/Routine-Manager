	...
	export default {
		...
		methods: {
			...
			getFormattedCourses() {
				...
				signedData.Courses.forEach(course => {
					this.Courses.push({
						Code: course.split("(")[0],
						Section: course.split("(")[1].split(")")[0]
					});
				});
			},
			onClickSave() {
				if (!this.checkIfHasAnyError()) {
					this.$store.dispatch(
						"updateCourses",
						this.ReFormatCourses(this.Courses)
					);
				} else {
					this.errorMassage = "Empty field error!";
				}
			},
			// updateCourses(){
